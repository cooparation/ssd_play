...
def do_python_eval(image_set, classes, confi_thresh, static_result, output_dir = 'results'):
    ...
    for i, cls in enumerate(classes):
        if cls == '__background__' or cls == 'background':
            continue
...
if __name__ == '__main__':
    ...
    #imageset_txt = "./build/image_lists.txt"
    imageset_txt = "../data/test_person.txt"
    output_dir = "./results"
    class_file = "./label_inf.txt"
    with open(class_file, "rb") as f:
        lines = f.readlines()

    classes = [t.strip("\n") for t in lines]

    static_result = open("./results/static.txt","a")
